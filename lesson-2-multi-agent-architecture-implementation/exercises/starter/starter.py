...
class PenguinAgent(ToolCallingAgent):
    ...
    def take_action(self) -> Dict[str, Any]:
        ...
        # ========================================
        # STUDENT TASK 2: Complete the penguin prompt
        # ========================================
        # Guide the LLM to make intelligent decisions about:
        # 1. When to use the 'find_food' tool (provide method: "fishing" or "foraging")
        # 2. When to request food from the scientist (respond with text like "I need food")
        # 3. When to request a toy from the scientist (respond with text like "I need a toy")
        # 
        # Consider the penguin's current state (food level, whether it has a toy)
        # Encourage the penguin to be self-sufficient when possible (use find_food)
